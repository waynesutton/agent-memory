...
import { memoryTypeValidator, scopeValidator, historyEventValidator, feedbackSentimentValidator, } from "./schema.js";
...
export const list = query({
    ...
    handler: async (ctx, args) => {
        ...
        if (args.minPriority !== undefined) {
            filtered = filtered.filter((m) => (m.priority ?? 0) >= args.minPriority);
        }
        if (args.tags && args.tags.length > 0) {
            filtered = filtered.filter((m) => args.tags.some((tag) => m.tags.includes(tag)));
        }
        if (args.createdAfter !== undefined) {
            filtered = filtered.filter((m) => m._creationTime > args.createdAfter);
        }
        if (args.createdBefore !== undefined) {
            filtered = filtered.filter((m) => m._creationTime < args.createdBefore);
        }
        ...
        if (direction === "from" || direction === "both") {
            let fromQuery = ctx.db
                ...
                if (args.relationship)
                    sq = sq.eq("relationship", args.relationship);
                return sq;
            });
