    ...
    handler: async (ctx, args) => {
        ...
        if (apiKey.rateLimitOverride) {
            ...
            const project = await ctx.db
                .query("projects")
                .withIndex("by_projectId", (q) => q.eq("projectId", apiKey.projectId))
                .first();
...
export const listApiKeys = query({
    ...
    handler: async (ctx, args) => {
        const keys = await ctx.db
            .query("apiKeys")
            .withIndex("by_project", (q) => q.eq("projectId", args.projectId).eq("revoked", false))
            .take(100);
        return keys.map((k) => ({
            _id: k._id,
            keyHash: k.keyHash,
...
//# sourceMappingURL=apiKeyQueries.js.map