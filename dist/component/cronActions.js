...
export const runDecayForAllProjects = internalAction({
    ...
    handler: async (ctx) => {
        ...
        const projects = await ctx.runQuery(internal.cronQueries.listDecayEnabledProjects);
        for (const project of projects) {
            const halfLifeDays = project.settings.decayHalfLifeDays ?? 30;
...
export const runHistoryCleanupForAllProjects = internalAction({
    ...
    returns: v.null(),
    handler: async (ctx) => {
        const projects = await ctx.runQuery(internal.cronQueries.listAllProjectIds);
...
//# sourceMappingURL=cronActions.js.map